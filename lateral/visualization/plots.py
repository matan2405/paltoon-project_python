...
def create_comprehensive_plots(sim_data: Dict, scenario_name: str = "Simulation", 
    ...
    fig, axes = plt.subplots(3, 3, figsize=IEEE_TALL_DOUBLE)
    fig.suptitle(scenario_name, fontsize=9)
    ...
    ax1 = axes[0, 0]
    ax1.plot(time, sim_data['human_y'], '-', linewidth=1.2, label=r'$y$')
    ax1.axhline(y=PLATOON_LANE_Y,       color='g', linestyle='--', linewidth=0.8, label='Target')
    ax1.axhline(y=HUMAN_INITIAL_LANE_Y, color='r', linestyle=':',  linewidth=0.8, alpha=0.6, label='Init')
    add_mobil_line(ax1)
    ax1.set_xlabel(r'$t$ [s]')
    ax1.set_ylabel(r'$y$ [m]')
    ax1.set_title('(a) Lateral position')
    ax1.legend(fontsize=8)
    ax1.grid(True, alpha=0.3)
    
    ax2 = axes[0, 1]
    ax2.plot(time, np.degrees(sim_data['human_psi']), '-', linewidth=1.2)
    ax2.axhline(y=0, color='g', linestyle='--', linewidth=0.8)
    add_mobil_line(ax2)
    ax2.set_xlabel(r'$t$ [s]')
    ax2.set_ylabel(r'$\psi$ [deg]')
    ax2.set_title(r'(b) Heading angle')
    ax2.grid(True, alpha=0.3)
    
    ax3 = axes[0, 2]
    ax3.plot(time, sim_data['human_y_dot_road'], '-',  linewidth=1.2, label=r'$\dot{Y}$ road')
    ax3.plot(time, sim_data['human_y_dot'],      '--', linewidth=0.9, alpha=0.7, label=r'$v_y$ body')
    ax3.axhline(y=0, color='k', linestyle='-', linewidth=0.4)
    add_mobil_line(ax3)
    ax3.set_xlabel(r'$t$ [s]')
    ax3.set_ylabel(r'$\dot{y}$ [m/s]')
    ax3.set_title(r'(c) Lateral velocity')
    ax3.legend(fontsize=7)
    ax3.grid(True, alpha=0.3)
    
    ax4 = axes[1, 0]
    ax4.plot(time, np.degrees(sim_data['delta_system']), '-',  linewidth=1.0, label=r'$u_1$ system')
    ax4.plot(time, np.degrees(sim_data['delta_human']),  '--', linewidth=1.0, label=r'$u_2$ human')
    ax4.plot(time, np.degrees(sim_data['delta_shared']), '-',  linewidth=1.4, label=r'$\delta_\mathrm{shared}$')
    add_mobil_line(ax4)
    ax4.set_xlabel(r'$t$ [s]')
    ax4.set_ylabel(r'$\delta$ [deg]')
    ax4.set_title(r'(d) Steering inputs')
    ...
    ax5.set_ylabel(r'$\lambda$ (log)')
    ax5.set_title(r'(e) Authority ratio $\lambda$')
    ax5.legend(fontsize=7, loc='upper right')
    ax5.grid(True, alpha=0.3)
    
    ax6 = axes[1, 2]
    ax6.plot(time, sim_data['field_force'], '-', linewidth=1.2)
    ax6.axhline(y=0, color='k', linestyle='-', linewidth=0.4)
    add_mobil_line(ax6)
    ax6.set_xlabel(r'$t$ [s]')
    ax6.set_ylabel(r'$F$ [N]')
    ax6.set_title('(f) DSF force')
    ax6.grid(True, alpha=0.3)
    
    ax7 = axes[2, 0]
    ax7.plot(time, sim_data['y_error'], '-', linewidth=1.2)
    ax7.axhline(y=0, color='g', linestyle='--', linewidth=0.8)
    add_mobil_line(ax7)
    ax7.set_xlabel(r'$t$ [s]')
    ax7.set_ylabel(r'$e_y$ [m]')
    ax7.set_title(r'(g) Lateral error')
    ax7.grid(True, alpha=0.3)
    
    ax8 = axes[2, 1]
    ax8.plot(time, sim_data['human_ay'], '-', linewidth=1.2)
    ax8.axhline(y= 2.5, color='r', linestyle='--', linewidth=0.8, alpha=0.5)
    ax8.axhline(y=-2.5, color='r', linestyle='--', linewidth=0.8, alpha=0.5)
    add_mobil_line(ax8)
    ax8.set_xlabel(r'$t$ [s]')
    ax8.set_ylabel(r'$a_y$ [m/s$^2$]')
    ax8.set_title(r'(h) Lateral acceleration')
...
def create_nash_analysis_plots(sim_data: Dict, scenario_name: str = "Simulation",
                               mobil_approval_time: float = None) -> plt.Figure:
    fig, axes = plt.subplots(2, 2, figsize=IEEE_DOUBLE_COL)
    fig.suptitle(f'Nash — {scenario_name}', fontsize=9)
    ...
    ax1 = axes[0, 0]
    ax1.plot(time, np.degrees(sim_data['delta_system']), '-',  linewidth=1.0, label=r'$u_1$ system')
    ax1.plot(time, np.degrees(sim_data['delta_human']),  '--', linewidth=1.0, label=r'$u_2$ human')
    ax1.plot(time, np.degrees(sim_data['delta_shared']), '-',  linewidth=1.4, label=r'$\delta_\mathrm{shared}$')
    add_mobil_line(ax1)
    ax1.set_xlabel(r'$t$ [s]')
    ax1.set_ylabel(r'$\delta$ [deg]')
    ax1.set_title('(a) Control inputs')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ...
    add_mobil_line(ax2)
    ax2.set_xlabel(r'$t$ [s]')
    ax2.set_ylabel(r'$\lambda$ (log)')
    ax2.set_title(r'(b) Authority ratio $\lambda$')
    ...
    ax3.set_title('(c) Authority split')
    ax3.set_ylim([0, 100])
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ...
    """
    ax4.text(0.1, 0.9, summary, transform=ax4.transAxes, fontsize=7,
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{scenario_name.replace(' ', '_')}_nash_{timestamp}.pdf"
    filepath = os.path.join(RESULTS_DIR, filename)
    save_fig(fig, filepath)
