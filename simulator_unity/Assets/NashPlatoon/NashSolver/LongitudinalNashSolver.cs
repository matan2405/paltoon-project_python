...
//
// Factory: call LongitudinalNashSolver.Create(w, t) — runs Setup immediately.
using System;
using OsqpSharp;
...
namespace NashPlatoon
{
    public sealed class LongitudinalNashSolver : NashSolverBase
    {
        ...
        OsqpSolver[] _s2;         // Player 2 — one per LambdaLevels entry (8 for long)
        ...
        public static LongitudinalNashSolver Create(NashWeightsConfig w, SimTimingConfig t)
        {
            var s = new LongitudinalNashSolver(w, t);
            s.InitDoubleIntegrator(t.NashDtLong);
            s.SetupSolvers(s.A, s.B);
            return s;
        }

        /// Create using vehicle's physical Jacobian (drag in A[1,1]) instead of pure double integrator.
        public static LongitudinalNashSolver CreateWithVehicle(
            NashWeightsConfig w, SimTimingConfig t, AdvancedBicycleModel vehicle)
        {
            var s = new LongitudinalNashSolver(w, t);
            s.C = new double[] { 1, 0, 0, 1 };
            s.Relinearize(vehicle, t.NashDtLong);
            return s;
        }

        // ── State-space initialisation ────────────────────────────────────────

        void InitDoubleIntegrator(float dt)
        {
            double d = dt;
            A = new double[] { 1, d, 0, 1 };            // 2×2 row-major
            B = new double[] { 0.5 * d * d, d };        // 2×1 (Nx×1)
            C = new double[] { 1, 0, 0, 1 };            // 2×2 = I₂
        }

        /// Rebuild prediction matrices from the vehicle's current linearisation.
        /// Call whenever vx changes significantly to keep the Nash model accurate.
        public void Relinearize(AdvancedBicycleModel vehicle, float dt)
        {
            var (Ac2d, Bc1d) = vehicle.GetLongJacobian();
            double[] Ac = { Ac2d[0,0], Ac2d[0,1], Ac2d[1,0], Ac2d[1,1] };
            double[] Bc = { (double)Bc1d[0], (double)Bc1d[1] };
            var (Ad, Bd) = MatrixOps.ZOH(Ac, Bc, 2, 1, dt);
            SetupSolvers(Ad, Bd);
        }
