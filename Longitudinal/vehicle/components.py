...
class Transmission:
    ...
    def get_total_ratio(self) -> float:
        """Get current total gear reduction, with smooth blending during shifts.

        During a gear shift the effective ratio transitions linearly from the
        old gear's ratio to the new gear's ratio over ``shift_duration`` seconds.
        This prevents the instantaneous force discontinuity that occurs when the
        ratio changes in one time-step.
        """
        if self.is_shifting:
            alpha = min(self.shift_timer / self.shift_duration, 1.0)
            return (1.0 - alpha) * self._old_total_ratio + alpha * self._new_total_ratio
